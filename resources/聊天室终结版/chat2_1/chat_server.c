...
void * service_thread( void* p ){
	...
	while(1){
		...
		int tv_ret = 0;
		int 	 tv_sock_err = 0;
		size_t   tv_err_len = sizeof( int );
		if( ( tv_ret = recv( fd,buf, sizeof(buf), 0 ) ) <= 0 ){//接受信息
			printf( "recv ret =%d", tv_ret );

			getsockopt( fd, SOL_SOCKET, SO_ERROR, &tv_sock_err, &tv_err_len );
			printf( "recv err =%d\n", tv_sock_err );
            if( tv_sock_err == EAGAIN ){
				printf("客户端等待\n");
				continue;
				//继续下一次循环，等待客户端连接
            }
		...
		if( strncmp( buf,"TF",2 ) == 0 ){//接收文件
			if(recv(fd,filename,sizeof(filename),0)<0){//接收文件名
				perror("recv filename");
					/*char fail[100] = {};
					sprintf(fail,"服务器接收文件%s失败",filename);
					send(fd,fail,strlen(fail),0);//把错误发给客户端*/
...
void service(){
	...
	while(1){
		...
		if(fd == -1){
			 getsockopt( sockfd, SOL_SOCKET, SO_ERROR, &tv_sock_err, &tv_err_len );
				printf( "accept err =%d\n", tv_sock_err );
                if( tv_sock_err != EAGAIN ){
					printf("客户端连接出错..\n");
		...
		// TODO 2.等待线程结束，并且获取退出信息,加入后会出问题：客户端不能连上来
		/*
		char* pc = NULL;
		pthread_join( pid,(void**)&pc );
